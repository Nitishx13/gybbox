...
export default function ReviewLinksClient({ clientSlug }: { clientSlug?: string }) {
  ...
  const createLink = (e: React.FormEvent) => {
    ...
    // Generate a shareable rate URL with slug baked in, if available
    ...
    const url = `${base}/rate${clientSlug ? `/${encodeURIComponent(clientSlug)}` : ""}${!clientSlug ? `?${params.toString()}` : params.toString() ? `?${params.toString()}` : ""}`;
    ...
    <div className="grid gap-4">
      ...
      {clientSlug && (
        <Card>
          ...
          <CardContent>
            <ClientFeedbackQuickForm clientSlug={clientSlug} />
          </CardContent>
        </Card>
      )}
    </div>
